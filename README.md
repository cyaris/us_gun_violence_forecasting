...
exposes `svelte-lib-ref`; automatic runs use `SVELTE_LIB_REF` when set and fall back to a dry-run build when the
repository has no AWS upload credentials configured. Production uploads require a pinned 40-character `svelte-lib`
commit SHA.
